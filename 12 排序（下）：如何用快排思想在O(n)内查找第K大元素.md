上一节我讲了**冒泡排序、插入排序、选择排序这三种排序算法，它们的时间复杂度都是O(n2)，比较高，适合小规模数据的排序**。今天，我讲两种时间复杂度为**O(nlogn)的排序算法，归并排序和快速排序。这两种排序算法适合大规模的数据排序**，比上一节讲的那三种排序算法要更常用。
...
![12-4 快排分区](images/12-4 快排分区.png)
